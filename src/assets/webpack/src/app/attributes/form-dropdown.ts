...
@customAttribute('blackcube-form-dropdown')
export class FormDropdown implements ICustomAttributeViewModel
{
    ...
    private initList() {
        ...
        Array.from(this.select.options)
            .forEach((option) => {
                ...
                const optGroupLabel = option.closest('optgroup')?.getAttribute('label');
                const textElement = htmlTemplateOption.querySelector('[data-value]') as HTMLElement;
                textElement.textContent = option.text;

                const textPrefixElement = htmlTemplateOption.querySelector('[data-value-prefix]') as HTMLElement;
                if (textPrefixElement) {
                    textPrefixElement.textContent = optGroupLabel;
                }
                ...
                if (option.selected) {
                    ...
                    this.logger.trace('Option selected', option.value);
                } else {
    ...
    private updateList(nofilter:boolean = undefined) {
        ...
        this.panel.querySelectorAll('[data-option-id]')
            .forEach((htmlButton: HTMLElement) => {
                const htmlLiOptions = htmlButton.closest('[data-form-dropdown="option"]') as HTMLElement;
                const text = htmlLiOptions.dataset.searchValue?.normalize('NFKD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
                let isFound = false;
                searchableValues.forEach((searchableValue) => {
                    const includedText = text.indexOf(searchableValue);
                    if ((nofilter === true) || (searchableValue === '') || includedText >= 0) { //text.includes(searchableValue)) {
                        isFound = true;
                    }
                });

                if (isFound) {
                    htmlLiOptions.classList.remove('hidden');
                } else {
                    htmlLiOptions.classList.add('hidden');
                }
                const check = htmlLiOptions.querySelector('[data-selected]')as HTMLElement;
                const selectedOption = this.select.querySelector(`option[value="${htmlButton.dataset.optionId}"]`) as HTMLOptionElement;
                check.dataset.selected = selectedOption.selected ? 'true' : 'false';
                // const option = this.select.query;
                if (selectedOption.selected) {
                    this.ariaService.setSelected(htmlButton, 'true');
                    check.classList.remove('hidden');
                } else {
                    this.ariaService.setSelected(htmlButton, 'false');
                    check.classList.add('hidden');
                }
                /*
    ...
    private onKeyPress = (evt: KeyboardEvent) => {
        if (evt.key === 'Tab' && !this.panel.classList.contains('hidden')) {
            ...
            this.ariaService.trapFocus(this.panel, this.input)
                .then((el: HTMLElement) => {
                    this.logger.trace('Focus trapped', el);
                });
